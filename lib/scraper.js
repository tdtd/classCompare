  ...
  Info.find({name: 'Bill'})
    ...
    .exec(function(err, found){
      if (found.length === 0){
        ...
        console.log('found.lastUpdate: '+found[0].lastUpdate);
        if (found[0].lastUpdate < yesterday) {
          console.log('updating too old');
          self.getAllSpecInfo()
            .then(function(result){
              found[0].specInfo = result;
              found[0].lastUpdate = Date.now();
              Info.findOneAndUpdate({name: 'Bill'}, found[0], {upsert:true}, function(err, doc){
                if (err) {
                  console.log(err);
                }
          ...
          console.log('getting it from mongo')
          self.classInfo = found[0].specInfo;
        }
      }
  });
};
